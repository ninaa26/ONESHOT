...
class TestHull:
    ...
    def test_zero_flow_zero_force(self, hull: LinearHydroModel, tf_tree: TFTree2D) -> None:
        """Hull should produce zero force with zero velocity."""
        state = make_state()

        fx, fy, _n = hull.compute(state, tf_tree)

        assert abs(fx) < 1e-6
        assert abs(fy) < 1e-6

    def test_forward_flow(self, hull: LinearHydroModel, tf_tree: TFTree2D) -> None:
    ...
    def test_quadrant_flow(self, hull: LinearHydroModel, u: float, v: float, tf_tree: TFTree2D) -> None:
